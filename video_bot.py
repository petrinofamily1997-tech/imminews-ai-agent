...
def make_subtitles(script, duration):
    ...
    # 2–3 words per subtitle = much easier to follow.
    groups = []
    current = []
    ...
    for word in words:
        ...
        # Prefer 3 words, but split earlier when the line gets too long.
        if len(current) >= 3 or len(text_now) >= 30:
    ...
    with open(subtitle_file, "w", encoding="utf-8") as f:
        ...
        for group in groups:
            ...
            # A tiny safety extension prevents flicker at word boundaries,
            # but never exceeds the real voice duration.
