...
def make_subtitles(script, duration):
    ...
    # ------------------------------------------------------------
    # IMPORTANT:
    # ElevenLabs gives us timestamps for every character.
    # We reconstruct every spoken word from those timestamps.
    #
    # Instead of grouping several words into one long subtitle,
    # each word gets its OWN subtitle event.
    #
    # This makes the narration the absolute master clock:
    # when the speaker starts a word -> that word appears.
    # when the next word starts -> the previous word disappears.
    #
    # Therefore subtitles cannot gradually "run ahead" of the voice
    ...
    #
    # Character alignment can sometimes give a word an end time that
    # slightly overlaps the following word. The next word's START is
    # the most reliable visual boundary for subtitles.
    #
    # This means:
    #   word 1: start_1 -> start_2
    #   word 2: start_2 -> start_3
    #   ...
    #
    # The last word ends at its real alignment end.
    # ------------------------------------------------------------
    ...
    for i, word in enumerate(words):
        ...
        # Ignore impossible/zero-length alignment artifacts.
    ...
    with open(subtitle_file, "w", encoding="utf-8") as f:
        ...
        for word in timed_words:
            # One spoken word = one subtitle.
            # The word is shown exactly for the time interval assigned
            # to it by ElevenLabs.
            caption = (
                r"{\b1\c&H00FFFFFF&}"
                + word["text"]
            ...
            f.write(
                f"Dialogue: 0,{ass_time(word['start'])},{ass_time(word['end'])},"
                f"Default,,0,0,0,,{caption}\n"
            )

    print(
        f"📝 Word-by-word subtitles synchronized to ElevenLabs timestamps: "
        f"{len(timed_words)} words."
    )
