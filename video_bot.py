...
FINAL_CTA = (
    ...
    "Если хочешь понимать не только новости, "
    "но и то, как они отражаются на твоих деньгах, "
    "переходи в наш Telegram-канал по ссылке в шапке профиля. "
    "Там мы разбираем финансовые новости, бизнес и экономику, "
    "делимся способами сохранить деньги и идеями для увеличения дохода."
)
...
def ensure_final_cta(script):
    ...
    wallet_phrase = "что это значит для твоего кошелька?"
    ...
    has_wallet = wallet_phrase in lower

    has_telegram = any(
        phrase in lower
        for phrase in telegram_phrases
    )

    # --------------------------------------------------------
    # If the model already created a proper CTA,
    # leave the script untouched.
    # --------------------------------------------------------

    if has_wallet and has_telegram:
        return script

    # --------------------------------------------------------
    # Remove an incomplete CTA from the end if the model
    # started one but did not finish it.
    # --------------------------------------------------------

    cleanup_patterns = [

        r"Что это значит для твоего кошелька\?.*$",
...
Финальная часть сценария ОБЯЗАТЕЛЬНО должна
постепенно подвести зрителя к вопросу:

"Что это значит для твоего кошелька?"

Эта фраза должна быть отдельной
финальной смысловой точкой перед CTA.

Сразу ПОСЛЕ неё обязательно должна идти
естественная рекомендация перейти
...
CTA должен звучать спокойно,
как продолжение полезного контента,
а не как агрессивная реклама.

В CTA можно упомянуть:
...
Вместо этого используй естественный переход
от пользы к Telegram-каналу.

ВАЖНО:

Фраза "Что это значит для твоего кошелька?"
должна обязательно присутствовать дословно.

Также обязательно должны присутствовать
слова "Telegram-канал" или "телеграм-канал"
и фраза "в шапке профиля".
...
В финале сделай плавную смысловую цепочку:

проблема
→ последствия
→ влияние на обычного человека
→ влияние на его деньги
→ "Что это значит для твоего кошелька?"
→ естественный переход в Telegram-канал
→ упоминание, что ссылка находится в шапке профиля.

Финал НЕ должен выглядеть как рекламная вставка.
    ...
    for model in GROQ_MODELS:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                # Increased because gpt-oss models
                # may need more completion space.
