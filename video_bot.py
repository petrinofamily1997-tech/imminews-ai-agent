...
Нужен готовый текст для озвучки вертикального ролика.
Стиль: интеллектуальный, уверенный, немного мрачный,
провокационный и философский, но естественный.

Тема: деньги, психология денег, финансовые ошибки,
инфляция, банки, инвестиции, заработок, расходы или экономика.

Начни с очень сильной мысли, которая цепляет в первые секунды.
Потом раскрой идею простым языком и закончи неожиданным выводом.
В конце добавь короткий призыв подписаться.
...
"""
    ...
    user_prompt = f"""
Создай один законченный сценарий для финансового Shorts/TikTok.

Стиль: {style}

Сделай текст разговорным, плотным и интересным.
Каждое предложение должно продвигать мысль.
Не добавляй заголовок или пояснения — верни только сценарий.
"""

    # GPT-OSS требует особого обращения с reasoning:
    # при include_reasoning=False финальный текст остаётся
    # в message.content, а reasoning не забирает вывод.
    models = [
    ...
    for model, extra in models:
        ...
        try:
            ...
            # Некоторые версии SDK могут вернуть текст как
            # список content-блоков.
            ...
            if not content:
                # Диагностика: не печатаем reasoning пользователю,
                # но показываем, что ответ действительно был без content.
                reasoning = getattr(message, "reasoning", None)
                if reasoning:
                    print(
                        "⚠️ Model returned reasoning but no final text."
                    )
                else:
                    print("⚠️ Groq returned empty text.")
                continue
            ...
            # Удаляем случайные служебные префиксы.
            content = re.sub(
