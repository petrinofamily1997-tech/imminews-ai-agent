...
# Duration of photo ad at the end
PHOTO_DURATION = 3.0
...
def create_photo_clip(photo_path, output_path, duration):
    """Creates a video clip from a static photo with specified duration"""
...
def create_visual_track(
    ...
    # --------------------------------------------------------
    # Calculate duration for main video (without photo ad)
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Create photo ad clip
    # --------------------------------------------------------
    
    photo_video = WORK_DIR / "photo_clip.mp4"
    if not create_photo_clip(PHOTO_FILE, photo_video, PHOTO_DURATION):
        print("⚠️ Не удалось создать рекламный кадр, используем только основное видео")
        return main_video

    # --------------------------------------------------------
    # Concatenate main video and photo ad
    # --------------------------------------------------------
...
def create_video(
    ...
    # Voice is the master clock + photo ad
    target_duration = (
        voice_duration
        + VOICE_TAIL
        + PHOTO_DURATION
    ...
    # ============================================================
    # STEP 1: Add voice to video (without music)
    # ============================================================
    ...
    if music and music.exists():
        ...
        music_converted = WORK_DIR / "music_converted.aac"
        cmd_convert = [
            "ffmpeg", "-y",
            "-i", str(music),
            "-vn",
        ...
        try:
            ...
            music_converted = None
        
        if music_converted and music_converted.exists():
            # Mix audio
            cmd_mix = [
                "ffmpeg", "-y",
                "-i", str(video_with_voice),
                "-i", str(music_converted),
                "-filter_complex",
