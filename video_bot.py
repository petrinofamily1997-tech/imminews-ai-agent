...
VOICE_TAIL = 0.90
PHOTO_DURATION = 3.0  # Длительность показа рекламной картинки в конце
...
def check_environment():
    ...
    # Проверяем наличие рекламного фото
    if not PHOTO_FILE.exists():
        print(f"❌ Рекламное фото не найдено: {PHOTO_FILE}")
        return False
...
Например по смыслу:
"Потому что знать новость мало. Гораздо важнее понимать, что она означает для собственных денег. Что это значит для твоего кошелька? Такие разборы, новости бизнеса и экономики, рекомендации по сохранению денег и идеи для увеличения дохода мы публикуем в нашем Telegram-канале."

Но НЕ копируй этот пример дословно. Создай свой естественный переход.
...
НЕ используй служебные слова: HOOK, CTA, INTRO, OUTRO, SCENE, SFX, MUSIC, ПАУЗА, ОЗВУЧКА. Они не должны попасть в SCRIPT.

Не давай персональных инвестиционных рекомендаций.
Не говори конкретно покупать или продавать акции, криптовалюты или другие активы.
Не обещай гарантированный заработок.

Длина SCRIPT: примерно 120–170 слов.
Текст должен естественно звучать в глубокой мужской озвучке.

После сценария создай от 10 до 14 разнообразных поисковых запросов Pexels.
Каждый запрос должен описывать отдельную визуальную сцену.
Очень важно: не повторяй одинаковые сцены.
Запросы должны быть короткими, на английском языке.
...
Не используй текстовые надписи или изображения с конкретными логотипами в самих запросах.
...
Очень важно: Ответ должен быть ИМЕННО в таком формате. Не используй JSON. Не используй Markdown. Не добавляй никакого текста до TITLE или после списка PEXELS.
"""
...
Главная задача: сделать сценарий интересным настолько, чтобы зритель не пролистал ролик.

Особенно хорошо раскрой, как эта тема связана с деньгами обычного человека.

В самом конце обязательно: логически подведи к вопросу "Что это значит для твоего кошелька?".
    ...
    try:
        ...
        for video in videos:
            ...
            for file in files:
                ...
                if link:
                    candidates.append((link, width, height))
        
        if not candidates:
            return None
        
        candidates.sort(key=lambda item: abs(item[1] - 1080))
        best = candidates[:min(7, len(candidates))]
...
def create_photo_clip(photo_path, output_path, duration):
    """Создает видео-клип из статичного фото с указанной длительностью"""
    print(f"🖼️ Создание рекламного кадра из {photo_path.name}...")
    ...
    try:
        subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        print(f"✅ Рекламный кадр создан: {output_path.name}")
        return True
    except subprocess.CalledProcessError as e:
        print("❌ Ошибка создания рекламного кадра:")
        print(e.stderr[-2000:] if e.stderr else "")
        return False
...
def create_visual_track(clips, duration):
    ...
    for index, clip in enumerate(clips):
        ...
        if prepare_clip(clip, output, MAX_CLIP_SECONDS + 1.0):
            prepared.append(output)
    
    if not prepared:
        return None
    
    # Определяем количество кадров для основной части видео
    # Оставляем место для рекламного кадра в конце
    main_duration = duration - PHOTO_DURATION
    if main_duration < 5.0:
        main_duration = duration * 0.7  # Если слишком коротко, оставляем 70% на основу
    
    desired_count = int(main_duration / random.uniform(MIN_CLIP_SECONDS, MAX_CLIP_SECONDS))
    desired_count = max(MIN_CLIPS, desired_count)
    desired_count = min(MAX_CLIPS, desired_count)
    
    # Создаем последовательность клипов
    visual_clips = []
    index = 0
    while len(visual_clips) < desired_count:
        candidate = prepared[index % len(prepared)]
        if visual_clips and candidate == visual_clips[-1] and len(prepared) > 1:
            index += 1
            candidate = prepared[index % len(prepared)]
    ...
    # Создаем длины сегментов
    raw_lengths = []
    for _ in range(count):
        raw_lengths.append(random.uniform(MIN_CLIP_SECONDS, MAX_CLIP_SECONDS))
    raw_total = sum(raw_lengths)
    segment_lengths = [x * main_duration / raw_total for x in raw_lengths]
    segment_lengths = [max(1.25, x) for x in segment_lengths]
    
    factor = main_duration / sum(segment_lengths)
    segment_lengths = [x * factor for x in segment_lengths]
    
    # Строим фильтр для FFmpeg
    filter_parts = []
    input_index = 0
    ...
    # Теперь создаем рекламный кадр из фото
    photo_video = WORK_DIR / "photo_clip.mp4"
    if not create_photo_clip(PHOTO_FILE, photo_video, PHOTO_DURATION):
        print("⚠️ Не удалось создать рекламный кадр, пропускаем...")
        return visual_track  # Возвращаем только основное видео
    
    # Соединяем основной трек и рекламный кадр
    final_visual = WORK_DIR / "visual_track_with_photo.mp4"
    concat_file = WORK_DIR / "concat_list.txt"
    
    with open(concat_file, "w") as f:
        f.write(f"file '{visual_track.absolute()}'\n")
    ...
    try:
        subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        print(f"🎞️ Visual duration: {get_duration(final_visual):.3f}s")
        print(f"🎬 Number of visual fragments: {count} + 1 (реклама)")
        return final_visual
    except subprocess.CalledProcessError as e:
        print("❌ Ошибка соединения видео с рекламой:")
        print(e.stderr[-2000:] if e.stderr else "")
        return visual_track  # Возвращаем хотя бы основное видео
...
def create_video(clips, music):
    ...
    # Увеличиваем длительность видео на время показа рекламного кадра
    target_duration = voice_duration + VOICE_TAIL + PHOTO_DURATION
    print(f"🎙️ Voice: {voice_duration:.3f}s")
    print(f"🎬 Target video: {target_duration:.3f}s (включая рекламу)")
    ...
    print(f"🖼️ Реклама Telegram: добавлена в конец видео")
    
    # Проверка безопасности
    if final_duration + 0.05 < voice_duration + PHOTO_DURATION:
        print("❌ SAFETY CHECK FAILED! Video is shorter than voice + photo.")
        return False
    
    print("✅ Все компоненты успешно объединены.")
    return True
...
def main():
    ...
    print("📹 Video ready:")
    print(FINAL_VIDEO)
    print("🖼️ В конце видео добавлена реклама Telegram-канала")
    print()
