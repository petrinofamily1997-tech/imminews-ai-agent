...
def create_video(clips, music):
    ...
    if visual_duration + 0.10 < target_duration:
        print("❌ Visual track is shorter than target duration.")
        return False

    # Render the main video first.  This deliberately separates audio
    # mixing from the Telegram advertisement concat, making each stage
    # independently testable.
    if music:
        # Music is looped as an FFmpeg input.  We use a minimal, portable
        # filter graph: voice is padded to target length, music is lowered,
        # and the two streams are mixed for the exact target duration.
        filter_complex = (
            f"[1:a]aresample=44100,apad,atrim=duration={target_duration:.6f},"
            "asetpts=PTS-STARTPTS[voice];"
            f"[2:a]aresample=44100,volume={MUSIC_VOLUME:.6f},"
            "aloop=loop=-1:size=2147483647,"
            f"atrim=duration={target_duration:.6f},asetpts=PTS-STARTPTS[music];"
            "[voice][music]amix=inputs=2:duration=first:dropout_transition=0:"
            "normalize=0,aresample=44100[aout]"
        )
    ...
    try:
        ...
        print(e.stderr[-10000:] if e.stderr else "")
        return False
    ...
    if main_duration + 0.10 < voice_duration:
        print("❌ Main video is shorter than voice.")
        return False

    # Append the Telegram end card as a separate, normalized MP4.
    ...
    if final_duration + 0.20 < main_duration + AD_DURATION:
        print("❌ SAFETY CHECK FAILED: advertisement was not appended correctly.")
        return False
