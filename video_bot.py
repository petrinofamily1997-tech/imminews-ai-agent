...
def fetch_free_news_topic():
    ...
    headers = {
        "User-Agent":
            "Mozilla/5.0 FinancialVideoBot/4.0"
    }
...
def validate_cta(script):
    ...
    # Главный вопрос должен присутствовать.
    if "что это значит для твоего кошелька?" not in lower:

        return False

    # После вопроса должен быть Telegram CTA.
    question_pos = lower.find(
    ...
    if not any(
        ...
        for word in telegram_words
    ):

        return False

    # Должна быть понятная ссылка на шапку профиля.
    profile_words = [
        "шапке профиля",
        "шапка профиля",
        "в профиле",
...
Создай ОДИН готовый сценарий на русском языке.
...
Длина примерно 120-170 слов.

Первые 1-2 предложения должны сразу
зацепить внимание.
...
Сделай неожиданный вывод.

Главная задача:
показать зрителю, как тема влияет
именно на его деньги.

ФИНАЛ ОЧЕНЬ ВАЖЕН.

Перед главным вопросом должна быть
естественная подводка от темы к личным
деньгам зрителя.

Затем ОБЯЗАТЕЛЬНО дословно произнеси:

"Что это значит для твоего кошелька?"

Эта фраза должна быть отдельным
смысловым предложением.

СРАЗУ ПОСЛЕ НЕЁ продолжи спокойным
естественным CTA.

В CTA обязательно объясни, что более
подробные финансовые разборы, новости
бизнеса и экономики, идеи по сохранению
денег и увеличению дохода публикуются
в Telegram-канале.

В конце обязательно должно быть понятно,
что Telegram-канал находится
в шапке профиля.

Например по смыслу:

"Если хочешь понимать такие изменения
раньше и видеть больше финансовых разборов,
новостей бизнеса и экономики, полезных
рекомендаций и идей для роста дохода,
всё это мы публикуем в нашем Telegram-канале.
Ссылка на него находится в шапке профиля."

Но НЕ копируй пример дословно.

CTA должен быть спокойным,
не агрессивным и не похожим на рекламу.
...
Не используй списки внутри текста сценария.

Не используй служебные обозначения.
...
Не говори человеку конкретно покупать
или продавать акции, криптовалюты
или другие активы.
...
Создай 10-14 коротких разнообразных
поисковых запросов на английском языке.

Каждый запрос должен соответствовать
отдельной визуальной сцене.

Не повторяй одну и ту же сцену.

Используй реальные кинематографичные
сцены, связанные с деньгами, бизнесом,
банками, экономикой, работой, покупками,
телефоном, офисами, городом и финансами.

Не используй логотипы брендов.

Не используй текстовые надписи.

Верни только данные согласно заданной
структуре.
"""
