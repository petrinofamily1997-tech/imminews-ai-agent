...
def make_subtitles(script, duration):
    ...
    if not alignment_file.exists():
        raise RuntimeError("alignment.json was not created by forced alignment.")
    ...
    if not raw_words:
        raise RuntimeError("Forced alignment returned no usable words.")

    # Use ONLY word-level timings from the forced-alignment pass.
    # We deliberately do not calculate timing from character durations.
    words = []
    for item in raw_words:
        text = str(item.get("text", "")).strip()
        if not text or item.get("type") not in (None, "word"):
            continue
        try:
        ...
        if end <= start:
            continue
        words.append({"text": text, "start": max(0.0, start), "end": min(duration, end)})

    if not words:
        raise RuntimeError("Could not build timed words from forced alignment.")

    # Build readable phrase cues, but NEVER invent timing.
    # Every cue starts at the first spoken word and ends at the last spoken word.
    cues = []
    current = []
    current_chars = 0
    MAX_CUE_WORDS = 6
    ...
    for word in words:
        proposed = current_chars + (1 if current else 0) + len(word["text"])
        if current and (len(current) >= MAX_CUE_WORDS or proposed > MAX_CUE_CHARS):
            flush()
        current.append(word)
        current_chars += (1 if len(current) > 1 else 0) + len(word["text"])

        # Prefer ending a cue at natural punctuation.
        if word["text"][-1:] in ".!?;:":
            flush()

    flush()

    # Safety: never allow a subtitle cue to extend beyond the actual audio.
    for cue in cues:
    ...
    with open(subtitle_file, "w", encoding="utf-8") as f:
        ...
        for cue in cues:
            ...
            f.write(
                ...
                f"Default,,0,0,0,,{{\b1\c&H00FFFFFF&}}{text}{{\b0}}\n"
            )

    last_subtitle_end = cues[-1]["end"] if cues else 0.0
    print(
        f"📝 Forced-alignment subtitles: {len(cues)} cues, "
