...
def generate_script():
    ...
    system_prompt = """
Ты сценарист коротких финансовых видео на русском языке.

Создай один законченный сценарий для вертикального видео.
Стиль: умный, спокойный, холодный, уверенный, слегка
провокационный, философский, с лёгким сарказмом.

Не копируй конкретных персонажей, блогеров, актёров или их голоса.

Тема: деньги, финансовые ошибки, заработок, инвестиции,
экономика, банки, инфляция, психология денег или расходы.

Структура:
сильный hook → интрига → идея/факт → неожиданный вывод →
короткий призыв подписаться.

Не начинай с «Сегодня мы поговорим», «В этом видео» или «Привет».
Не используй Markdown, **, URL, хэштеги или эмодзи.
Не обещай гарантированную прибыль.
Не давай персональных инвестиционных рекомендаций.

ОЧЕНЬ ВАЖНО:
Ответ должен содержать ТОЛЬКО готовый текст сценария.
Никаких JSON.
Никаких заголовков TITLE/SCRIPT.
Никаких пояснений.
Примерная длина: 100–140 слов.
"""

    user_prompt = f"""
Напиши готовый сценарий финансового видео.

Выбранный стиль: {style}

Сделай его таким, чтобы человек остановил ленту
на первых двух предложениях и захотел досмотреть до конца.
Текст должен естественно звучать при глубокой мужской озвучке.
"""
    ...
    for model in GROQ_MODELS:
        ...
        try:
            ...
            # GPT-OSS иногда добавляет служебное оформление.
            content = re.sub(
                r"```(?:text|markdown)?",
                "",
                content,
            ...
            # Не обрезаем нормальный сценарий посередине.
            if len(words) > 155:
                content = " ".join(words[:155])
                content = content.rstrip(".,!?;:") + "."

            title = (
                "Финансовая мысль, о которой "
                "вам не расскажут"
            )
