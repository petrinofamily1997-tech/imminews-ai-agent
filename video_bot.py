...
MAIN_VIDEO = WORK_DIR / "video_main.mp4"
AD_IMAGE = BASE_DIR / "assets" / "photo.png"
AD_VIDEO = WORK_DIR / "telegram_ad.mp4"
...
# We want a lot of visual changes.
MIN_CLIPS = 8
MAX_CLIPS = 14

# Target duration of one visual fragment.
MIN_CLIP_SECONDS = 1.6
MAX_CLIP_SECONDS = 3.0

# Safety time after the final spoken word.
VOICE_TAIL = 0.90

# Telegram channel promo appended after the main video.
AD_DURATION = 4.0
...
def check_environment():
    ...
    if not AD_IMAGE.is_file():

        print(f"❌ Telegram ad image not found: {AD_IMAGE}")
...
После этого естественно свяжи тему
с деньгами самого зрителя.

В ФИНАЛЕ обязательно должна быть логическая
цепочка:

проблема или новость
→ влияние на обычного человека
→ деньги зрителя
→ фраза "Что это значит для твоего кошелька?"
→ объяснение, что подобные разборы доступны
...
должна звучать естественно и быть отдельной
финальной смысловой точкой.

После неё обязательно скажи, что
в нашем Telegram-канале можно узнать:
...
CTA должен быть спокойным и естественным.

Например по смыслу:

"Потому что знать новость мало.
Гораздо важнее понимать, что она означает
для собственных денег.

Что это значит для твоего кошелька?

Такие разборы, новости бизнеса и экономики,
рекомендации по сохранению денег и идеи
для увеличения дохода мы публикуем
в нашем Telegram-канале."

Но НЕ копируй этот пример дословно.

Создай свой естественный переход.
...
логически подведи к вопросу
"Что это значит для твоего кошелька?"

После вопроса скажи, что подобные
финансовые разборы, новости бизнеса
и экономики, рекомендации по сохранению денег
и идеи по увеличению дохода есть
в нашем Telegram-канале.

CTA не должен выглядеть как реклама.
...
def _save_and_validate_audio(content):
...
def _generate_voice_edge_tts(tts_script):

    """Fallback TTS that does not consume ElevenLabs quota."""

    try:
        import asyncio
        import edge_tts

        print("🔁 Trying Edge TTS fallback (ru-RU-DmitryNeural)...")
        ...
        duration = get_duration(AUDIO_FILE)
        if duration <= 0:
            print("❌ Edge TTS produced invalid audio.")
            return False

        print(f"✅ Edge TTS fallback generated: {duration:.2f}s")
        return True
...
def generate_voice(script):
    ...
    try:
        ...
        if response.status_code == 200:
            return _save_and_validate_audio(response.content)

        print(f"❌ ElevenLabs error {response.status_code}")
        print(response.text[:1500])

        # A quota error must not kill the whole GitHub Actions run.
        # Use Edge TTS, which does not spend ElevenLabs credits.
        if response.status_code == 401 and "quota_exceeded" in response.text.lower():
            print("⚠️ ElevenLabs quota exceeded. Switching to free TTS fallback...")
            return _generate_voice_edge_tts(tts_script)

        return False

    except Exception as e:
        print(f"❌ ElevenLabs error: {e}")
        print("⚠️ ElevenLabs request failed. Trying TTS fallback...")
        return _generate_voice_edge_tts(tts_script)
...
def create_visual_track(clips, duration):
    """Create a reliable 9:16 visual track exactly as long as duration.

    Every source clip is looped at the FFmpeg input level, so a short
    Pexels clip can never make the final montage unexpectedly shorter.
    """
    print("🎞️ Creating dynamic montage...")

    if not clips:
        print("❌ No visual clips supplied.")
        return None

    # Use enough cuts to keep the video dynamic, but never exceed the
    # number of available unique clips too aggressively.
    desired_count = int(
        duration / random.uniform(MIN_CLIP_SECONDS, MAX_CLIP_SECONDS)
    )
    desired_count = max(MIN_CLIPS, desired_count)
    desired_count = min(MAX_CLIPS, desired_count)
    desired_count = max(1, desired_count)

    visual_clips = []
    index = 0
    while len(visual_clips) < desired_count:
        candidate = clips[index % len(clips)]
        if (
            visual_clips
            ...
            index += 1
            candidate = clips[index % len(clips)]
        visual_clips.append(candidate)
        index += 1

    count = len(visual_clips)

    # Generate segment lengths and normalize them so they sum EXACTLY to
    # the requested duration.
    raw_lengths = [
        random.uniform(MIN_CLIP_SECONDS, MAX_CLIP_SECONDS)
        for _ in range(count)
    ]
    raw_total = sum(raw_lengths)
    segment_lengths = [x * duration / raw_total for x in raw_lengths]

    # Do not allow tiny fragments; normalize again afterwards.
    segment_lengths = [max(1.25, x) for x in segment_lengths]
    factor = duration / sum(segment_lengths)
    segment_lengths = [x * factor for x in segment_lengths]

    command = ["ffmpeg", "-y"]

    # Critical fix: loop EVERY source input indefinitely.  The old version
    # prepared clips to ~4 seconds and then asked FFmpeg for longer trims,
    # causing concat to end around 59s instead of the requested 102s.
    for clip in visual_clips:
        command += ["-stream_loop", "-1", "-i", str(clip)]

    filter_parts = []

    for i, segment in enumerate(segment_lengths):
        zoom_mode = random.choice(["center", "left", "right"])
        if zoom_mode == "left":
            crop_x = "0"
    ...
    filter_parts.append(
        f"{joined}concat=n={count}:v=1:a=0,"
    ...
    try:
        ...
        print("❌ Visual montage error:")
        print(e.stderr[-8000:] if e.stderr else "")
        return None

    visual_track = WORK_DIR / "visual_track.mp4"
    if not visual_track.exists():
        print("❌ Visual track was not created.")
        return None

    actual_duration = get_duration(visual_track)
    print(f"🎞️ Visual duration: {actual_duration:.3f}s")
    print(f"🎬 Number of visual fragments: {count}")

    # Hard check before the final render.
    if actual_duration + 0.10 < duration:
        print(
            f"❌ Visual track is too short: "
            f"{actual_duration:.3f}s < {duration:.3f}s"
        )
        return None
...
def append_telegram_ad(main_video):

    """Append assets/photo.png as a short end-card to the rendered video."""

    print("📣 Appending Telegram advertisement...")

    if not AD_IMAGE.is_file():
        print(f"❌ Advertisement image not found: {AD_IMAGE}")
        return False

    # The ad is encoded with the same vertical format and a silent audio
    # track so the two segments can be concatenated reliably.
    ad_command = [
        "ffmpeg",
        "-y",
        "-loop", "1",
        ...
        (
            ...
            f"fade=t=out:st={max(0.35, AD_DURATION - 0.35):.3f}:d=0.35"
        ),
        "-r", str(FPS),
        "-c:v", "libx264",
        "-preset", "veryfast",
    ...
    try:
        ...
        print("❌ Advertisement render error:")
        print(e.stderr[-5000:] if e.stderr else "")
        return False

    if not AD_VIDEO.exists():
        print("❌ Advertisement video was not created.")
        return False

    concat_list = WORK_DIR / "concat.txt"
    concat_list.write_text(
        "file '" + str(main_video.resolve()).replace("'", "'\\''") + "'\n"
        "file '" + str(AD_VIDEO.resolve()).replace("'", "'\\''") + "'\n",
        encoding="utf-8",
    )
    ...
    try:
        ...
        print("❌ Video + advertisement concat error:")
        print(e.stderr[-7000:] if e.stderr else "")
        return False
    ...
    print(
        f"📣 Telegram ad appended: {final_duration - main_duration:.2f}s"
    )
    print(f"📺 Final duration with ad: {final_duration:.2f}s")

    # Make sure the ad actually increased the duration.
    return final_duration >= main_duration + AD_DURATION - 0.20
...
def create_video(clips, music):
    """Render the main video and append the Telegram end card.

    This version deliberately avoids the fragile escaped FFmpeg audio graph
    that was causing:
        No such filter: ''
    in GitHub Actions.
    """
    ...
    # The main video lasts exactly as long as the narration.
    # The advertisement is appended afterwards as a separate MP4.
    target_duration = voice_duration

    print(f"🎙️ Voice: {voice_duration:.3f}s")
    print(f"🎬 Main video target: {target_duration:.3f}s")
    print(f"📣 Advertisement: {AD_DURATION:.3f}s")

    visual_track = create_visual_track(clips, target_duration)
    if not visual_track:
        print("❌ Could not create visual track.")
        return False

    visual_duration = get_duration(visual_track)
    if visual_duration + 0.10 < target_duration:
        print(
            f"❌ Visual track is shorter than target: "
            f"{visual_duration:.3f}s < {target_duration:.3f}s"
        )
        return False

    # --------------------------------------------------------
    # STEP 1: Prepare music as a standalone AAC track.
    # No filter_complex is used here.
    # --------------------------------------------------------
    music_track = None
    if music and Path(music).is_file():
        music_track = WORK_DIR / "music_loop.m4a"
        ...
        try:
            ...
            print("⚠️ Music preparation failed. Continuing with voice only.")
            print(e.stderr[-6000:] if e.stderr else "")
            music_track = None
    ...
    #
    # IMPORTANT:
    # FFmpeg receives arguments directly as a subprocess list.
    # Therefore NO backslash escaping such as [1\:a] is allowed.
    #
    # Music and voice are already normalized standalone inputs.
    # --------------------------------------------------------
    if music_track and music_track.exists():
        # The graph is intentionally tiny and contains no escaped labels.
        audio_filter = (
            "[0:a][1:a]"
    ...
    try:
        ...
        print("❌ FFmpeg main render error:")
        print(e.stderr[-12000:] if e.stderr else "")
        return False

    if not MAIN_VIDEO.exists():
        print("❌ Main video was not created.")
        return False

    main_duration = get_duration(MAIN_VIDEO)
    print(f"✅ Main video: {main_duration:.3f}s")

    if main_duration + 0.15 < voice_duration:
        print(
            f"❌ Main video is shorter than voice: "
            f"{main_duration:.3f}s < {voice_duration:.3f}s"
        )
        return False

    # --------------------------------------------------------
    # STEP 3: Append Telegram advertisement.
    # --------------------------------------------------------
    if not append_telegram_ad(MAIN_VIDEO):
        print("❌ Telegram advertisement could not be appended.")
        return False
    ...
    print(f"🎙️ Voice: {voice_duration:.3f}s")
    print(f"🎬 Main video: {main_duration:.3f}s")
    print(f"📣 Telegram advertisement: {AD_DURATION:.3f}s")
    print(f"🎬 Final video: {final_duration:.3f}s")
    ...
    if final_duration + 0.20 < main_duration + AD_DURATION:
        print(
            "❌ SAFETY CHECK FAILED: advertisement was not appended correctly."
        )
        return False

    print("✅ Voice is fully contained inside video.")
    print("✅ Telegram advertisement is present at the end.")
    print("🚫 NO SUBTITLES WERE ADDED.")
