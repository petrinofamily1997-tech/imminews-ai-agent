...
if required_question not in lower:
    ...
    print(
        "⚠️ CTA question is missing."
    )

    return False

question_position = lower.rfind(
    required_question
)
...
after_question = lower[
    question_position
    + len(required_question):
].strip()

# Telegram must be AFTER the question.
if (
    "telegram" not in after_question
    and "телеграм" not in after_question
):

    print(
        "⚠️ Telegram CTA is missing after the question."
    )

    return False

# The CTA should mention the profile header.
profile_words = [
    "шапке профиля",
    "шапка профиля",
    "в профиле",
...
if not any(
    word in after_question
    for word in profile_words
):

    print(
        "⚠️ Profile CTA is missing."
    )

    return False

# Prevent the question from appearing at the
# very beginning of the script.
if question_position < len(lower) * 0.45:

    print(
        "⚠️ CTA question appeared too early."
    )
...
Ты профессиональный сценарист коротких финансовых видео.

Пиши только на русском языке.

Создавай естественные сценарии для TikTok и YouTube Shorts.

Стиль:
умный, спокойный, уверенный, холодный,
аналитичный, слегка провокационный.

Не копируй конкретных блогеров.

Главная цель:
удержать внимание зрителя и объяснить,
как финансовая тема влияет на обычного человека.

СТРУКТУРА:

1. Сильный хук в первых секундах.
2. Раскрытие проблемы или новости.
3. Простое объяснение.
4. Конкретный жизненный пример.
5. Неожиданный вывод.
6. Плавная связь с деньгами зрителя.
7. Финальная смысловая связка.
8. Спокойный переход в Telegram.

КРИТИЧЕСКИ ВАЖНО:

В конце сценария обязательно должна быть именно фраза:

"Что это значит для твоего кошелька?"

Перед этой фразой должна быть естественная подводка.

Например по смыслу:

"И вот здесь становится важен не сам заголовок,
а то, как это отразится на обычных расходах
и доходах человека.

Что это значит для твоего кошелька?"

После вопроса обязательно должен идти спокойный CTA.

Обязательно объясни, что в нашем Telegram-канале
выходят финансовые разборы, новости бизнеса
и экономики, рекомендации по сохранению денег
и идеи по увеличению дохода.

Обязательно скажи, что Telegram-канал находится
в шапке профиля.

Финальные предложения должны звучать естественно,
например по смыслу:

"Если хочешь понимать такие изменения раньше
и видеть не только новости, но и их последствия
для своих денег, такие разборы мы публикуем
в нашем Telegram-канале. Ссылка на канал
находится в шапке профиля."

НЕ копируй этот пример дословно.

Важно:

Фраза "Что это значит для твоего кошелька?"
должна быть отдельной смысловой точкой.

После неё обязательно должен быть Telegram CTA.

CTA не должен быть агрессивной рекламой.
...
Текст должен хорошо звучать
в глубокой мужской озвучке.

После SCRIPT создай от 10 до 14 коротких
поисковых запросов Pexels на английском языке.

Каждый запрос должен описывать отдельную сцену.
...
Не добавляй никакого текста до TITLE
или после PEXELS.

НЕ возвращай JSON.

НЕ используй structured output.
...
Сделай сценарий максимально естественным
для русской мужской озвучки.

Особенно хорошо покажи,
как тема связана с деньгами обычного человека.

В финале обязательно сделай:

естественная подводка
→ "Что это значит для твоего кошелька?"
→ спокойный переход в Telegram
→ объяснение пользы канала
→ упоминание, что ссылка на Telegram-канал
находится в шапке профиля.

Важно:
после вопроса "Что это значит для твоего кошелька?"
не заканчивай сценарий сразу.

После вопроса обязательно должно быть
ещё несколько предложений про Telegram.
...
for model in GROQ_MODELS:
    ...
    try:
        ...
        # ------------------------------------------------
        # IMPORTANT:
        # Do NOT request JSON / structured output.
        # GPT-OSS models can fail validation when forced
        # into structured generation for this plain-text
        # parser.
        # ------------------------------------------------
