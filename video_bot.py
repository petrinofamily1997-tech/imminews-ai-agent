...
def create_visual_track(clips, target_duration):
    ...
    if not prepared:
        print("❌ No clips were prepared")
        return None
    
    # Определяем длительность основной части (без рекламы)
    main_duration = target_duration - PHOTO_DURATION
    if main_duration < 5.0:
        main_duration = target_duration * 0.7
    
    # Создаем простой список клипов с повторениями
    simple_clips = []
    for _ in range(min(MAX_CLIPS, max(MIN_CLIPS, int(main_duration / 2.0)))):
        clip = random.choice(prepared)
        simple_clips.append(clip)
    ...
    try:
        subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        print(f"✅ Main video created: {main_video.name}")
    except subprocess.CalledProcessError as e:
        print("❌ Main video creation error:")
        print(e.stderr[-2000:] if e.stderr else "")
        return None
    
    # Создаем рекламный кадр
    ...
    concat_file2 = WORK_DIR / "concat_list2.txt"
    ...
    try:
        subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        visual_duration = get_duration(final_visual)
        print(f"🎞️ Visual duration: {visual_duration:.3f}s")
        print(f"🎬 Количество клипов: {len(simple_clips)} + 1 (реклама)")
        return final_visual
    except subprocess.CalledProcessError as e:
...
def create_video(clips, music):
    ...
    if not visual_track or not visual_track.exists():
        print("❌ Could not create visual track.")
        return False
    
    # Проверяем длительность визуального трека
    visual_duration = get_duration(visual_track)
    if visual_duration <= 0:
        print(f"❌ Invalid visual track duration: {visual_duration}")
        return False
    
    print(f"📊 Visual track duration: {visual_duration:.3f}s")
    
    # Финальный рендеринг
    if music and music.exists():
        print("🎵 Добавляем голос и музыку...")
        command = [
            "ffmpeg", "-y",
            "-i", str(visual_track),
            "-i", str(AUDIO_FILE),
            ...
            "-filter_complex",
            (
                f"[1:a]atrim=0:{voice_duration:.3f},asetpts=PTS-STARTPTS[voice];"
                f"[2:a]volume={MUSIC_VOLUME},atrim=0:{target_duration:.3f},asetpts=PTS-STARTPTS[music];"
                "[voice][music]amix=inputs=2:duration=longest[finalaudio]"
            ),
            "-map", "0:v:0",
            "-map", "[finalaudio]",
    ...
    try:
        subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        print("✅ FFmpeg render successful")
    except subprocess.CalledProcessError as e:
