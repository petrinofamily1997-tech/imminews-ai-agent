...
def create_video(clips, music):
    ...
    target_duration = voice_duration + VOICE_TAIL

    print(f"🎙️ Voice: {voice_duration:.3f}s")
    print(f"🎬 Target video: {target_duration:.3f}s (без рекламы)")
    ...
    # --------------------------------------------------------
    # Normalize music first.
    # This avoids the fragile multi-input filter graph that was
    # producing "No such filter: ''" in GitHub Actions.
    # --------------------------------------------------------
    music_track = WORK_DIR / "music_loop.mp3"
    ...
    if music:
        ...
        try:
            ...
            print("⚠️ Music preparation failed. Continuing without music.")
            print(e.stderr[-5000:] if e.stderr else "")
            music_track = None
    ...
    #
    # IMPORTANT:
    # Do not use the old escaped filter graph:
    # [1\:a] ... [2\:a]
    #
    # We first create a clean music track, then use FFmpeg's
    # amerge/amix only once with normal labels.
    # --------------------------------------------------------
    ...
    if main_duration + 0.10 < voice_duration:
        print(
    ...
    # --------------------------------------------------------
    # Append Telegram advertisement as a separate normalized
    # video. This keeps the ad isolated from the main audio.
    # --------------------------------------------------------
    ...
    if final_duration + 0.10 < voice_duration:
