...
def parse_script_response(text):
    ...
    if not script:
        return None
    return {"title": title, "script": script, "pexels_queries": queries}
...
def generate_script():
    ...
    style = random.choice(
        ["PROVOCATIVE", "INTELLECTUAL", "DARK", "PHILOSOPHICAL", "ANALYTICAL"]
    )
    topic = choose_topic()
...
luxury shopping
grocery shopping
Очень важно: Ответ должен быть ИМЕННО в таком формате.
Не используй JSON.
Не используй Markdown.
Не добавляй никакого текста до TITLE или после списка PEXELS.
"""
    user_prompt = f"""
...
логически подведи к вопросу "Что это значит для твоего кошелька?"
...
def create_visual_track(clips, duration):
    ...
    if not prepared:
        return None
    desired_count = int(duration / random.uniform(MIN_CLIP_SECONDS, MAX_CLIP_SECONDS))
    ...
    filter_parts.append(
        f"{joined}concat=n={count}:v=1:a=0,format=yuv420p[v]"
    )
    command = ["ffmpeg", "-y"]
...
def append_telegram_ad(main_video):
    ...
    print(f"📣 Telegram ad appended: {final_duration - main_duration:.2f}s")
...
def create_video(clips, music):
    ...
    # --------------------------------------------------------
    if music:
        # ИСПРАВЛЕНО: единая f-строка без лишних запятых и разрывов, ломающих синтаксис FFmpeg
        filter_complex = (
            f"[1:a]aresample=44100,apad,atrim=0:{voice_duration:.6f},asetpts=N/SR/TB[voice];"
            f"[2:a]aresample=44100,volume={MUSIC_VOLUME},apad,atrim=0:{target_duration:.6f},asetpts=N/SR/TB[music];"
            "[voice][music]amix=inputs=2:duration=longest:dropout_transition=0[finalaudio]"
        )
        command = [
            "ffmpeg",
            "-y",
    ...
    if final_duration + 0.05 < voice_duration:
        ...
        print("Video is shorter than voice.")
        return False
        
    print("✅ Voice is fully contained inside video.")
    print(f"📣 Telegram advertisement: {AD_IMAGE}")
...
def main():
    ...
    if len(clips) < MIN_CLIPS:
        print("⚠️ Not enough clips.")
        print(f"Required: {MIN_CLIPS}")
