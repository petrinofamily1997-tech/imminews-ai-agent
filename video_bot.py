...
ASSETS_DIR = BASE_DIR / "assets"  # ДОБАВЛЕНО

AUDIO_FILE = WORK_DIR / "voice.mp3"
FINAL_VIDEO = OUTPUT_DIR / "video.mp4"
PHOTO_FILE = ASSETS_DIR / "photo.png"  # ДОБАВЛЕНО
...
VOICE_TAIL = 0.90
PHOTO_DURATION = 3.0  # ДОБАВЛЕНО - длительность показа рекламы
...
def check_environment():
    ...
    if PHOTO_FILE.exists():
        print(f"✅ Рекламное фото найдено: {PHOTO_FILE}")
    else:
        print(f"⚠️ Рекламное фото не найдено: {PHOTO_FILE} (продолжаем без рекламы)")
...
# ============================================================
# CREATE PHOTO CLIP - НОВАЯ ФУНКЦИЯ
# ============================================================

def create_photo_clip(photo_path, output_path, duration):
    """Создает видео-клип из статичного фото"""
...
# ============================================================
# CREATE DYNAMIC VISUAL TRACK - ИЗМЕНЕНО
# ============================================================
...
def create_visual_track(
    ...
    # --------------------------------------------------------
    # Вычитаем время на фото из общей длительности
    # --------------------------------------------------------
    
    main_duration = duration - PHOTO_DURATION
    if main_duration < 3.0:
    ...
    # If we have fewer downloaded clips,
    # reuse them in different positions.
    visual_clips = []
    ...
    # --------------------------------------------------------
    # Create slightly different segment lengths.
    # --------------------------------------------------------
    ...
    segment_lengths = [
        ...
        x * main_duration / raw_total

        for x in raw_lengths

    ]

    # Prevent extremely tiny fragments.
    segment_lengths = [
    ...
    if PHOTO_FILE.exists():
        ...
        if create_photo_clip(PHOTO_FILE, photo_video, PHOTO_DURATION):
            # Соединяем основное видео и фото
            final_visual = WORK_DIR / "visual_track.mp4"
            ...
            try:
                subprocess.run(command_concat, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
                actual_duration = get_duration(final_visual)
                print(f"🎞️ Visual duration: {actual_duration:.3f}s")
                print(f"🎬 Number of visual fragments: {count} + 1 (реклама)")
                return final_visual
            except subprocess.CalledProcessError as e:
                print("⚠️ Не удалось добавить рекламу, используем только основное видео")
                return main_video
        else:
            return main_video
    else:
        print("⚠️ Рекламное фото не найдено, видео без рекламы")
        return main_video


# ============================================================
# FINAL VIDEO - ПОЛНОСТЬЮ ПЕРЕПИСАНА
# ============================================================
...
def create_video(
    ...
    # Voice is the master clock + время на фото
    target_duration = (
        voice_duration
        + VOICE_TAIL
        + PHOTO_DURATION  # ДОБАВЛЕНО
    )
    ...
    print(
        f"🎬 Target video: "
        f"{target_duration:.3f}s (включая рекламу)"
    )
    ...
    # ============================================================
    # ШАГ 1: Добавляем голос к видео (без музыки) - УПРОЩЕННЫЙ ПОДХОД
    # ============================================================
    
    print("🎤 Шаг 1: Добавляем голос к видео...")
    video_with_voice = WORK_DIR / "video_with_voice.mp4"
    ...
    try:
        ...
        print("✅ Голос добавлен")
    except subprocess.CalledProcessError as e:
        print("❌ Ошибка добавления голоса:")
        print(e.stderr[-2000:] if e.stderr else "")
        return False

    # ============================================================
    # ШАГ 2: Если есть музыка, добавляем её - УПРОЩЕННЫЙ ПОДХОД
    # ============================================================
    
    if music and music.exists():
        print("🎵 Шаг 2: Добавляем музыку...")
        
        # Конвертируем музыку в правильный формат
        ...
        try:
            ...
            print("✅ Музыка сконвертирована")
        except:
            print("⚠️ Не удалось конвертировать музыку, пропускаем...")
            music_aac = None
        ...
        if music_aac and music_aac.exists():
            ...
            try:
                ...
                print(f"⚠️ Не удалось добавить музыку, сохраняем без неё")
                shutil.copy2(video_with_voice, FINAL_VIDEO)
        else:
            shutil.copy2(video_with_voice, FINAL_VIDEO)
    else:
        print("🎵 Без музыки")
    ...
    if PHOTO_FILE.exists():
        print(
            f"🖼️ Реклама Telegram: добавлена в конец видео"
        )
    else:
        print(
            f"⚠️ Реклама не добавлена (фото отсутствует)"
        )
...
def main():
    ...
    if PHOTO_FILE.exists():
        print(
            "🖼️ В конце видео добавлена реклама Telegram-канала"
        )
